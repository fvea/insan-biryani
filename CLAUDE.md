...
State lives in two module-level JS variables, each backed by a `localStorage` key, loaded once in the `init()` IIFE and re-persisted on every mutation:

| Variable   | localStorage key | Holds                         | Save via            |
|------------|------------------|-------------------------------|---------------------|
| `menuData` | `ib_menu`        | categories + items            | `saveMenuData()`    |
| `log`      | `ib_log`         | all sales records             | `localStorage.setItem('ib_log', ...)` |
...
- **Cart and menu match items by `name`.** Two items sharing a name (even across categories) collide. Keep this in mind for any item-identity logic.
